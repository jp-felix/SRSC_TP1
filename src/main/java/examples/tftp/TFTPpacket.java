...
public class TFTPpacket {
    ...
    public static TFTPpacket receive(DSTPSocket dstpSocket) throws Exception {
        TFTPpacket in = new TFTPpacket();
        TFTPpacket retPak = new TFTPpacket();

        // Receive data securely using DSTPSocket
        byte[] receivedData = dstpSocket.receive();
        System.arraycopy(receivedData, 0, in.message, 0, receivedData.length);
        in.length = receivedData.length;
        ...
        retPak.length = in.length;
        // IMPORTANT: took out host and port info because they will need to be managed in `DSTPSocket` or passed along with data

        return retPak;
    }

    //Method to send packet
    public void send(InetAddress ip, int port, DSTPSocket dstpSocket) throws Exception {
        dstpSocket.send(message, ip, port);
    }
    ...
    protected void put(int at, String value, byte del) {
        System.arraycopy(value.getBytes(), 0, message, at, value.length());
